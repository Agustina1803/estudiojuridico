...
const FormSubirArchivo = ({ mostrar, cerrar }) => {
  ...
  const SubirArchivo = (data) => {
    ...
    const docsExistentes = JSON.parse(localStorage.getItem("documentos") || "[]");
    docsExistentes.push(data);
    localStorage.setItem("documentos", JSON.stringify(docsExistentes));
      ...
      <Modal.Body>
        <Form onSubmit={handleSubmit(SubirArchivo)}>
          ...
          <Stack direction="horizontal" gap={3}>
            <Form.Control
              className="me-auto"
              placeholder="Añade aquí tu archivo"
            />
            <Button variant="success" type="file" className="me-2">
              Subir
            </Button>
            <div className="vr" />
